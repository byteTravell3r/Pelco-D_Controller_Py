...
class PELCOD_CONTROLLER():
    ...
    def SET_SERIAL(self, COM):
        self.COM_PORT = COM
        try: self.SERIAL = serial.Serial(self.COM_PORT, 9600, timeout=0.1)
    ...
    def SET_ANGLE(self, DIR, ANGLE):
        ...
        match DIR:
            case "H": 
                ...
                return self.SEND_CMD(self.ADDR, 0x4B, self.HPOS[1], self.HPOS[0])
            case "V":
                if ANGLE > 0x863E: ANGLE = 0x863E
                self.VPOS = [int(ANGLE % 0x100), int(ANGLE / 0x100)]
                return self.SEND_CMD(self.ADDR, 0x4D, self.VPOS[1], self.VPOS[0])
            case _  : return "INVALID COMMAND"
    ...
    def QUERY_ANGLE(self, DIR):
        ...
        if DIR == "H": self.HPOS = [self.RFRAME[3], self.RFRAME[4]]
        if DIR == "V": self.VPOS = [self.RFRAME[3], self.RFRAME[4]]
        return 0
